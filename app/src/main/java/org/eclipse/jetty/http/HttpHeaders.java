...
public class HttpHeaders extends BufferCache {
    ...
    /* renamed from: TE */
    public static final String f4590TE = "TE";
    ...
    static {
        ...
        ACCEPT_BUFFER = CACHE.add("Accept", 19);
        ACCEPT_CHARSET_BUFFER = CACHE.add("Accept-Charset", 20);
        ACCEPT_ENCODING_BUFFER = CACHE.add("Accept-Encoding", 21);
        ACCEPT_LANGUAGE_BUFFER = CACHE.add("Accept-Language", 22);
        CONTENT_LENGTH_BUFFER = CACHE.add("Content-Length", 12);
        CONNECTION_BUFFER = CACHE.add("Connection", 1);
        CACHE_CONTROL_BUFFER = CACHE.add("Cache-Control", 57);
        DATE_BUFFER = CACHE.add("Date", 2);
        PRAGMA_BUFFER = CACHE.add("Pragma", 3);
        TRAILER_BUFFER = CACHE.add("Trailer", 4);
        TRANSFER_ENCODING_BUFFER = CACHE.add("Transfer-Encoding", 5);
        UPGRADE_BUFFER = CACHE.add("Upgrade", 6);
        VIA_BUFFER = CACHE.add("Via", 7);
        WARNING_BUFFER = CACHE.add("Warning", 8);
        ALLOW_BUFFER = CACHE.add("Allow", 9);
        CONTENT_ENCODING_BUFFER = CACHE.add("Content-Encoding", 10);
        CONTENT_LANGUAGE_BUFFER = CACHE.add("Content-Language", 11);
        CONTENT_LOCATION_BUFFER = CACHE.add("Content-Location", 13);
        CONTENT_MD5_BUFFER = CACHE.add("Content-MD5", 14);
        CONTENT_RANGE_BUFFER = CACHE.add("Content-Range", 15);
        ...
        EXPECT_BUFFER = CACHE.add("Expect", 24);
        FORWARDED_BUFFER = CACHE.add(FORWARDED, 25);
        FROM_BUFFER = CACHE.add("From", 26);
        IF_MATCH_BUFFER = CACHE.add("If-Match", 28);
        IF_MODIFIED_SINCE_BUFFER = CACHE.add("If-Modified-Since", 29);
        IF_NONE_MATCH_BUFFER = CACHE.add("If-None-Match", 30);
        IF_RANGE_BUFFER = CACHE.add("If-Range", 31);
        IF_UNMODIFIED_SINCE_BUFFER = CACHE.add("If-Unmodified-Since", 32);
        KEEP_ALIVE_BUFFER = CACHE.add("Keep-Alive", 33);
        MAX_FORWARDS_BUFFER = CACHE.add("Max-Forwards", 34);
        PROXY_AUTHORIZATION_BUFFER = CACHE.add("Proxy-Authorization", 35);
        RANGE_BUFFER = CACHE.add("Range", 36);
        REQUEST_RANGE_BUFFER = CACHE.add(REQUEST_RANGE, 37);
        REFERER_BUFFER = CACHE.add("Referer", 38);
        TE_BUFFER = CACHE.add("TE", 39);
        USER_AGENT_BUFFER = CACHE.add("User-Agent", 40);
        X_FORWARDED_FOR_BUFFER = CACHE.add(X_FORWARDED_FOR, 41);
        X_FORWARDED_PROTO_BUFFER = CACHE.add(X_FORWARDED_PROTO, 59);
        X_FORWARDED_SERVER_BUFFER = CACHE.add(X_FORWARDED_SERVER, 60);
        X_FORWARDED_HOST_BUFFER = CACHE.add(X_FORWARDED_HOST, 61);
        ACCEPT_RANGES_BUFFER = CACHE.add("Accept-Ranges", 42);
        AGE_BUFFER = CACHE.add("Age", 43);
        ETAG_BUFFER = CACHE.add("ETag", 44);
        LOCATION_BUFFER = CACHE.add("Location", 45);
        PROXY_AUTHENTICATE_BUFFER = CACHE.add("Proxy-Authenticate", 46);
        RETRY_AFTER_BUFFER = CACHE.add("Retry-After", 47);
        SERVER_BUFFER = CACHE.add("Server", 48);
        SERVLET_ENGINE_BUFFER = CACHE.add(SERVLET_ENGINE, 49);
        VARY_BUFFER = CACHE.add("Vary", 50);
        WWW_AUTHENTICATE_BUFFER = CACHE.add("WWW-Authenticate", 51);
        COOKIE_BUFFER = CACHE.add(COOKIE, 52);
        SET_COOKIE_BUFFER = CACHE.add(SET_COOKIE, 53);
        SET_COOKIE2_BUFFER = CACHE.add(SET_COOKIE2, 54);
        MIME_VERSION_BUFFER = CACHE.add(MIME_VERSION, 55);
        IDENTITY_BUFFER = CACHE.add("identity", 56);
        PROXY_CONNECTION_BUFFER = CACHE.add(PROXY_CONNECTION, 58);
    }
}
