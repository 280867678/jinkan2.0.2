...
public abstract class LocalGENASubscription extends GENASubscription<LocalService> implements PropertyChangeListener {
    public static Logger log = Logger.getLogger(LocalGENASubscription.class.getName());
    public final List<URL> callbackURLs;
    public final Map<String, Long> lastSentTimestamp = new HashMap();
    public final Map<String, Long> lastSentNumericValue = new HashMap();
    ...
    public LocalGENASubscription(LocalService localService, Integer num, List<URL> list) throws Exception {
        ...
        Logger logger = log;
        StringBuilder Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww = outline.Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww("Got evented state variable values: ");
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww.append(currentState.size());
        ...
        for (StateVariableValue stateVariableValue : currentState) {
            ...
            if (log.isLoggable(Level.FINEST)) {
                Logger logger2 = log;
                StringBuilder Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww2 = outline.Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww("Read state variable value '");
                Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww2.append(stateVariableValue.getStateVariable().getName());
                Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww2.append("': ");
                Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww2.append(stateVariableValue.toString());
        ...
        StringBuilder Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww3 = outline.Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww("uuid:");
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww3.append(UUID.randomUUID());
        this.subscriptionId = Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww3.toString();
    ...
    public synchronized Set<String> moderateStateVariables(long j, Collection<StateVariableValue> collection) {
        ...
        for (StateVariableValue stateVariableValue : collection) {
            ...
            if (stateVariable.getEventDetails().getEventMaximumRateMilliseconds() == 0 && stateVariable.getEventDetails().getEventMinimumDelta() == 0) {
                logger = log;
                str = "Variable is not moderated: " + stateVariable;
            } else if (!this.lastSentTimestamp.containsKey(name)) {
                logger = log;
                str = "Variable is moderated but was never sent before: " + stateVariable;
            } else {
                if (stateVariable.getEventDetails().getEventMaximumRateMilliseconds() > 0 && j <= this.lastSentTimestamp.get(name).longValue() + stateVariable.getEventDetails().getEventMaximumRateMilliseconds()) {
