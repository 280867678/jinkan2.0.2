...
public class MutableDevice {
    ...
    public Service[] createServices(Device device) throws ValidationException {
        Service[] mo5199newServiceArray = device.mo5199newServiceArray(this.services.size());
        int i = 0;
        for (MutableService mutableService : this.services) {
            mo5199newServiceArray[i] = mutableService.build(device);
            i++;
        }
        return mo5199newServiceArray;
    }
}
