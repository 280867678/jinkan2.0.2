...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initPath(Path path) {
        ...
        this.f152mX = new float[min];
        this.f153mY = new float[min];
        float[] fArr = new float[2];
        for (int i2 = 0; i2 < min; i2++) {
            pathMeasure.getPosTan((i2 * length) / (min - 1), fArr, null);
            this.f152mX[i2] = fArr[0];
            this.f153mY[i2] = fArr[1];
        }
        if (Math.abs(this.f152mX[0]) <= 1.0E-5d && Math.abs(this.f153mY[0]) <= 1.0E-5d) {
            int i3 = min - 1;
            if (Math.abs(this.f152mX[i3] - 1.0f) <= 1.0E-5d && Math.abs(this.f153mY[i3] - 1.0f) <= 1.0E-5d) {
                float f = 0.0f;
                int i4 = 0;
                while (i < min) {
                    float[] fArr2 = this.f152mX;
                    int i5 = i4 + 1;
                    float f2 = fArr2[i4];
                    if (f2 < f) {
        ...
        StringBuilder Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww = outline.Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww("The Path must start at (0,0) and end at (1,1) start: ");
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww.append(this.f152mX[0]);
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww.append(",");
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww.append(this.f153mY[0]);
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww.append(" end:");
        int i6 = min - 1;
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww.append(this.f152mX[i6]);
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww.append(",");
        Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww.append(this.f153mY[i6]);
    ...
    private void parseInterpolatorFromTypeArray(TypedArray typedArray, XmlPullParser xmlPullParser) {
        if (TypedArrayUtils.hasAttribute(xmlPullParser, "pathData")) {
            ...
            if (createPathFromPathData == null) {
                throw new InflateException(outline.Wwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwwww("The path is null, which is created from ", namedString));
            }
    ...
    public float getInterpolation(float f) {
        ...
        int i = 0;
        int length = this.f152mX.length - 1;
        while (length - i > 1) {
            int i2 = (i + length) / 2;
            if (f < this.f152mX[i2]) {
                length = i2;
        ...
        float[] fArr = this.f152mX;
        float f2 = fArr[length] - fArr[i];
        if (f2 == 0.0f) {
            return this.f153mY[i];
        }
        float[] fArr2 = this.f153mY;
        float f3 = fArr2[i];
