...
public class Matrix4f {
    ...
    private float computeCofactor(int i, int i2) {
        ...
        int i13 = i4 + i6;
        int i14 = i5 + i6;
        float f = (((fArr[i13] * fArr[i12]) - (fArr[i8] * fArr[i14])) * fArr[i3 + i9]) + ((((fArr[i8] * fArr[i10]) - (fArr[i11] * fArr[i12])) * fArr[i3 + i6]) - (((fArr[i13] * fArr[i10]) - (fArr[i11] * fArr[i14])) * fArr[i7 + i3]));
        return ((i + i2) & 1) != 0 ? -f : f;
    }
    ...
    public boolean inverseTranspose() {
        ...
        float f = fArr[0];
        float[] fArr2 = matrix4f.mMat;
        float f2 = (fArr[4] * fArr2[4]) + (f * fArr2[0]);
        float f3 = (fArr[12] * fArr2[12]) + (fArr[8] * fArr2[8]) + f2;
        if (Math.abs(f3) < 1.0E-6d) {
            return false;
        }
        float f4 = 1.0f / f3;
        for (int i3 = 0; i3 < 16; i3++) {
            this.mMat[i3] = matrix4f.mMat[i3] * f4;
        }
        return true;
    }
    ...
    public void loadProjectionNormalized(int i, int i2) {
        ...
        matrix4f.loadFrustum(f2, f3, f4, f, 1.0f, 100.0f);
